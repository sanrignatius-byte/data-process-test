...
#SBATCH --output=logs/dr_ablation_%j.out
#SBATCH --error=logs/dr_ablation_%j.err

echo "============================================"
echo "Dense Retrieval Ablation: baseline vs v1 vs v2 vs v3"
...
AUG_DIR="data/05_eval/dense_retrieval/augmented"
OUT_DIR="data/05_eval/dense_retrieval"
QUERIES="$AUG_DIR/queries.jsonl"

# v1: enriched element text (same passage_ids, better text)
echo ""
...
python scripts/eval_dense_retrieval.py \
    ...
    --batch-size 8 --max-length 512 --top-k 100 --device cuda
echo "v1 done: $(date)"
