#!/bin/bash
#SBATCH --job-name=33_para_merge_step3_rerun
#SBATCH --output=logs/33_paragraph_merge_step3_rerun_%j.out
#SBATCH --error=logs/33_paragraph_merge_step3_rerun_%j.err
...
for OUT_DIR in "$OUT_N400" "$OUT_N500"; do
    ...
    echo ""
    echo "  Evaluating $TAG ..."
    python scripts/eval_dense_retrieval_hf.py \
        --corpus      "$CORPUS" \
        --qrels       "$QRELS" \
        --queries     "$OUT_DIR/queries.jsonl" \
        --model-name  "$MODEL" \
        --output      "$OUT_DIR/eval_v3_chunks_qwen06b.json" \
        --save-ranking "$OUT_DIR/ranking_v3_chunks_qwen06b.jsonl" \
        --data-dir    "$OUT_DIR" \
        --batch-size  8
done
