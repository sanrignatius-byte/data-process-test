...
> 三项工程修复（quality_score 重建 + hub coverage 扩大 + citation walk 方向修正）将 hub_overlap 从 9.53% 提升到 90.42%；组件权重解耦后关闭 citation_walk，graph_full 首次超越 BM25（MRR +0.0403, R@10 +0.0269），达到 continue_expand 阈值。核心贡献是 1-hop neighbor propagation（邻域标签传播），支撑 4 月专利申请的效果验证已达标。

---

## 日期：2026-03-17（专利化评审 + 文档修正）

### 一、背景

完成 GRAPH_ARCHITECTURE.md v3（汇报版）和 REPORT_SUMMARY_2026-03-17_zh.md 后，对两份文档进行了**多视角评审**：

1. 助手 A：从多个角度（专利 scope、公式防绕过、学术术语、权利要求结构、周报数据准确性、技术文档专利适用性）提出约 20 条建议
2. 助手 B（专利律师 + 审查人员双维度）：从新颖性 / 创造性 / 实用性三大专利性出发，给出系统性评价
3. 我方逐条判定对错，综合采纳后执行修改
...
| 建议 | 改动 |
|------|------|
| "零 LLM 成本"需要限定 scope | → "图构建与 rerank 全过程零 LLM 调用" |
| 自定义术语缺少法定定义 | → 新增术语定义表（8 个核心术语） |
| 技术问题界定偏应用场景 | → 重写为专利法层面技术问题表述 |
| LaTeX 绑定过重影响专利保护范围 | → 抽象为"显式引用标记"，LaTeX 降为实施例 |
| "显著超越"缺统计显著性检验支撑 | → 去掉"显著"二字 |
| neighbor propagation 公式有歧义 | → 修正为 `neighbor_boost(c) ← λ × max_{n∈N(c)} s_bm25(n)` |
| 缺少 PRF 对比 | → 新增 PRF 行 + 核心区别说明 |
| 现有技术引证不规范 | → 加 BM25/GraphRAG/PRF 文献引证 |
| "已知局限"不适合专利文本 | → 改为"改进方向"，调整语气 |
| hub coverage 分母不清晰 | → 明确标注 query-level（25/261 → 236/261） |
| TF-IDF dense 未解释 | → 加"TF-IDF 向量余弦相似度检索" |
| MoDora "代码完成"可能误导 | → 产出总览加"（待全量验证）" |
| 10 vs 11 queries 可能引发疑问 | → 加注释区分单组件 vs 组合 |

#### 被判定为错误的建议

| 建议 | 判定理由 |
|------|---------|
| hub coverage "分母变了" | ❌ 分母始终是 261 queries，9.53%=25/261, 90.42%=236/261，代码已验证 |
| "10 vs 11 数字不一致" | ❌ 10=neighbor_prop 单组件，11=graph_full 组合，不是同一指标 |
| "Bridge Hub 等英文概念名应去除" | ❌ 这是我们自定义术语，不是学术圈已有概念 |
| 助手 B 的 GraphRAG 引证"微软 2023 论文" | ❌ GraphRAG 核心论文是 2024 年的（Edge et al.） |

#### 有道理但不急于改的建议

| 建议 | 处理方式 |
|------|---------|
| 公式系数防绕过（bridge_score 15/2） | 正式专利文本时抽象为"第一权重×模态多样性+第二权重×出度" |
| 权利要求书三独权结构 | 正式撰写专利申请书时执行 |
| 补充替代实施例 | 正式专利文本时补充 |
| 装置类权利要求 | 正式专利申请时增加 |
| PCT 国际申请 | 国内发明专利优先，数据充分后再考虑 |

### 三、文档版本变化

| 文档 | 变化 |
|------|------|
| `GRAPH_ARCHITECTURE.md` | v3 → v4（13 处修改，+27 行） |
| `REPORT_SUMMARY_2026-03-17_zh.md` | 6 处修正 |

### 四、关键结论

1. **GRAPH_ARCHITECTURE.md v4 现在同时服务两个目的**：10 分钟汇报 + 专利素材底稿
2. **正式专利文本撰写时还需要做的事**：权利要求书提炼、替代实施例补充、装置类权利要求、公式系数抽象化
3. **两位助手的评审质量**：助手 A 约 60% 建议有效（部分判断错误如 hub coverage 分母、10 vs 11），助手 B 约 80% 建议有效（专利框架专业，但引证信息有误）
