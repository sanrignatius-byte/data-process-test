...
15. BRIDGE MUST CONNECT TO BOTH ENDPOINTS. The bridge evidence_span MUST mention at least one concrete content term (method name / mechanism / dataset / variable) that also appears in Node 1's caption/context AND at least one that appears in Node 3's caption/context. Bridges that only repeat Node 1's vocabulary (or only Node 3's) describe a single side and will be rejected automatically.
