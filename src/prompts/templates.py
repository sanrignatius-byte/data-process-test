...
9. The query MUST ask ONE serial causal/comparative target. Do NOT use "and what", "and which", "and under what", or comma+and to ask two parallel questions. When you need to reference BOTH endpoints, fold one of them into a premise clause ("Given that …", "When …", "For the … that …") so only ONE interrogative target remains. Apply the SPLIT-TEST SELF-CHECK above before finalizing. This rule is hard-enforced by QC — there is no "and what/and which" exception for any query_type.
10. The query MUST contain no numerals, percentages, exact ranges, dimensions, or exact metric values. This explicitly INCLUDES dimensional patterns such as `6×384`, `512-d`, `k×k`, `1-bit`, `8-bit`, `n-dim`, `3-hop` and bit-width / channel-count / rank values. Use qualitative descriptors instead ("higher-rank", "aggressively quantized", "wider embedding", "deeper variant"). Year tokens (1900–2099) and the literals 0/1 are exempt only when they are clearly not a metric value.
11. Do NOT use bare demonstrative "this", "that", or "here" in the query (e.g. "this metric", "that drop", "what happens here?"). Name the method/dataset/metric or use a grounded phrase such as "the low-resource curve" or "the final-stage row". Note: complementizer "that" inside a clause (e.g. "the curve shows that accuracy plateaus", "Given that the constraint bounds X") is allowed and is NOT what this rule forbids — only the bare demonstrative reading is rejected.
