...
Muhammad Bilal Zafar, Isabel Valera, Manuel Gomez Rodriguez, Krishna P. Gummadi Max Planck Institute for Software Systems (MPI-SWS) {mzafar, ivalera, manuelgr, gummadi}@mpi-sws.org
...
![](images/1610.08452_page0_fig0.jpg)
...
In a binary classification task, the goal is to learn a mapping $f ( \mathbf { x } )$ between user feature vectors $\mathbf { x } \in \mathbb { R } ^ { d }$ and class labels $y \in$ $\{ - 1 , 1 \}$ . Learning this mapping is often achieved by finding a decision boundary $\pmb { \theta } ^ { \ast }$ in the feature space that minimizes a certain loss $L ( \pmb \theta )$ , i.e., $\pmb { \theta } ^ { * } = \mathrm { a r g m i n } _ { \pmb { \theta } } L ( \pmb { \theta } )$ , computed on a training dataset $\mathcal { D } = \{ ( \mathbf { x } _ { i } , y _ { i } ) \} _ { i = 1 } ^ { N }$ . Then, for a given unseen feature vector $\mathbf { x }$ , the classifier predicts the class label $\hat { y } =$ $f _ { \pmb { \theta } ^ { * } } ( \mathbf { x } ) \ : = \ : 1$ if $d _ { \pmb { \theta } ^ { * } } ( \mathbf { x } ) \geq 0$ and $\hat { y } = - 1$ otherwise, where $d _ { \pmb { \theta } ^ { * } } \left( \mathbf { x } \right)$ denotes the signed distance from $\mathbf { x }$ to the decision boundary. Assume that each user has an associated sensitive feature $z$ . For ease of exposition, we assume $z$ to be binary, i.e., $z \in \{ 0 , 1 \}$ . However, our setup can be easily generalized to categorical as well as multiple sensitive features.
...
<table><tr><td rowspan="2" colspan="2"></td><td colspan="2">Predicted Label</td><td rowspan="2"></td></tr><tr><td>ŷ = 1</td><td>ŷ = -1</td></tr><tr><td rowspan="2">True Label</td><td>1 y</td><td>True positive</td><td>False negative</td><td>P(ŷ ≠ y|y = 1) 
False 
Negative Rate</td></tr><tr><td>-1 y</td><td>False positive</td><td>True negative</td><td>P(ŷ ≠ y|y = -1) 
False 
Positive Rate</td></tr><tr><td colspan="2"></td><td>P(ŷ ≠ y|ŷ = 1) 
False 
Discovery Rate</td><td>P(ŷ ≠ y|ŷ = -1) 
False 
Omission Rate</td><td>P(ŷ ≠ y) 
Overall 
Misclass. Rate</td></tr></table>

i.e., if the probability that the classifier outputs a specific value of $\hat { y }$ given a feature vector x does not change after observing the sensitive feature $z$ , there is no disparate treatment.
...
where the term $\mathbb { E } [ ( z - { \bar { z } } ) ] { \bar { g } } _ { \pmb { \theta } } ( \mathbf { x } )$ cancels out since $\mathbb { E } [ ( z - { \bar { z } } ) ] =$ 0 and the function $g _ { \boldsymbol { \theta } } ( y , \mathbf { x } )$ is defined as:
...
which approximates, respectively, the conditions in Eqs. (3- 5). Note that, if a decision boundary satisfies Eqs. (3-5), the covariance defined above for that boundary will be close to zero, i.e., $\operatorname { C o v } ( z , g _ { \pmb { \theta } } ( y , \mathbf { x } ) ) \approx 0$ . Moreover, in linear models for classification, such as logistic regression or linear SVMs, the decision boundary is simply the hyperplane defined by $\pmb { \theta } ^ { T } \mathbf { x } = 0$ , therefore, $d _ { \pmb { \theta } } ( \mathbf { x } ) = \pmb { \theta } ^ { T } \mathbf { x }$ .
...
where the covariance threshold $c \in \mathbb { R } ^ { + }$ controls how adherent to disparate mistreatment the boundary should be. 3
...
where $\sim$ may denote $\geq$ ’ or $\leq$ ’. Also, we drop the constant number $\textstyle { \frac { 1 } { N } }$ for the sake of simplicity. Since the sensitive feature $z$ is binary, i.e., $z \in \{ 0 , 1 \}$ , we can split the sum in the above expression into two terms:
...
where $\mathcal { D } _ { 0 }$ and $\mathcal { D } _ { 1 }$ are the subsets of the training dataset $\mathcal { D }$ taking values $z = 0$ and $z = 1$ , respectively. Define $N _ { 0 } =$ |D0| and N1 = |D1|, then one can write ¯z = (0×N0)+(1×N1) $| \mathcal { D } _ { 0 } |$ $N _ { 1 } = | D _ { 1 } |$ $\begin{array} { r } { \overline { { z } } = \frac { ( 0 \times N _ { 0 } ) + ( 1 \times N _ { 1 } ) } { N } = } \end{array}$ $\textstyle { \frac { N _ { 1 } } { N } }$ and rewrite (15) as:
...
which, given that $g _ { \boldsymbol { \theta } } ( y , \mathbf { x } )$ is convex in $\pmb { \theta }$ , results into a convex-concave (or, difference of convex) function.
...
which is a Disciplined Convex-Concave Program (DCCP) for any convex loss $L ( \pmb \theta )$ , and can be efficiently solved using well-known heuristics such as the one proposed by Shen et al. [26]. Next, we particularize the formulation given by (17) for a logistic regression classifier [6]. 4

# Logistic regression without disparate mistreatment.

In logistic regression, the optimal decision boundary $\pmb { \theta } ^ { * }$ can be found by solving a maximum likelihood problem of the form θ∗ = argminθ − PNi=1 $\begin{array} { r } { \pmb { \theta } ^ { * } \ = \ \mathrm { a r g m i n } _ { \pmb { \theta } } - \sum _ { i = 1 } ^ { N } \log p ( y _ { i } | \mathbf { x } _ { i } , \pmb { \theta } ) } \end{array}$ in the training phase. Hence, a fair logistic regressor can be trained by solving the following constrained optimization problem:
...
where the closer the values of $D _ { F P R }$ and $D _ { F N R }$ to 0, the lower the degree of disparate mistreatment.
...
![](images/1610.08452_page0_fig1.jpg)  
(a) Cov. vs FPR

![](images/1610.08452_page0_fig2.jpg)  
(b) Fairness vs. Acc.

![](images/1610.08452_page0_fig3.jpg)  
(c) Boundaries   
...
![](images/1610.08452_page0_fig4.jpg)  
(a) FPR constraints

![](images/1610.08452_page0_fig5.jpg)  
(b) FNR constraints

![](images/1610.08452_page0_fig6.jpg)  
(c) Both constraints

![](images/1610.08452_page0_fig7.jpg)  
...
![](images/1610.08452_page0_fig8.jpg)  
(b) FNR constraints

![](images/1610.08452_page0_fig9.jpg)  
(c) Both constraints   
Figure 4: [Synthetic data] $D _ { F P R }$ and $D _ { F N R }$ have the same sign. Removing disparate mistreatment on FPR can potentially increase disparate mistreatment on FNR. Removing disparate mistreatment on both at the same time causes a larger drop in accuracy.
...
Results. Table 2 (last block) summarizes the results by showing the trade-off between fairness and accuracy achieved by our method, the method by Hardt et al., and the baseline. Similarly to the results in Section 5.1.2, we observe that for all three mehtods, controlling for disparate mistreatment on false positive rate (false negative rate) also helps decrease disparate mistreatment on false negative rate (false positive rate). Moreover, all three methods are able to achieve similar accuracy for a given level of fairness.

Additionally, we observe that our method (as well as the baseline) does not completely remove disparate mistreatment, i.e., it does not achieve zero $D _ { F P R }$ or/and $D _ { F N R }$ in any of the cases. This is probably due to the relatively small size of the dataset 8 (and hence a smaller ratio between number of training examples and number of learnable features), which hinders a robust estimate of misclassification covariance (Eqs. 11 and 12). This highlights the fact that our method can suffer from reduced performance on small datasets. In scenarios with sufficiently large training datasets, we expect more reliable estimates of covariance,
...
<table><tr><td>0.660</td><td>0.06</td><td>-0.14</td><td>0.662</td><td>0.03</td><td>-0.10</td><td>0.661</td><td>0.03</td><td>-0.11</td></tr><tr><td>0.643</td><td>0.03</td><td>-0.11</td><td>0.660</td><td>0.00</td><td>-0.07</td><td>0.660</td><td>0.01</td><td>-0.09</td></tr><tr><td>0.659</td><td>0.02</td><td>-0.08</td><td>0.653</td><td>-0.06</td><td>-0.01</td><td>0.645</td><td>-0.01</td><td>-0.01</td></tr></table>

Table 3: Recidivism rates in ProPublica COMPAS data for both races.   

<table><tr><td>Race</td><td>Yes</td><td>No</td><td>Total</td></tr><tr><td>Black</td><td>1,661(52%)</td><td>1,514(48%)</td><td>3,175(100%)</td></tr><tr><td>White</td><td>8,22(39%)</td><td>1,281(61%)</td><td>2,103(100%)</td></tr><tr><td>Total</td><td>2,483(47%)</td><td>2,795(53%)</td><td>5,278(100%)</td></tr></table>

Table 4: Description of features used from ProPublica COMPAS data.   

<table><tr><td>Feature</td><td>Description</td></tr><tr><td>Age Category</td><td>&lt; 25, between 25 and 45, &gt; 45</td></tr><tr><td>Gender</td><td>Male or Female</td></tr><tr><td>Race</td><td>White or Black</td></tr><tr><td>Priors Count</td><td>0–37</td></tr><tr><td>Charge Degree</td><td>Misconduct or Felony</td></tr><tr><td>2-year-rec.</td><td rowspan="2">Whether (+ve) or not (-ve) the defendant recidivated within two years</td></tr><tr><td>(target feature)</td></tr></table>

and hence, a better performance from our method. On the other hand, the method by Hardt et al. is able to achieve both zero $D _ { F P R }$ and $D _ { F N R }$ while controlling for disparate mistreatment on both false positive and false negative rates (Table 2)—albeit at a considerable drop in terms of accuracy. Since this method operates on a data of much smaller dimensionality (the final classifier probability estimates), it is not expected to suffer as much from the small size of the dataset as compared to our method or the baseline (which depend on the misclassification covariance computed on the feature set).
...
[4] J. Angwin, J. Larson, S. Mattu, and L. Kirchner. Machine Bias: There’s Software Used Across the Country to Predict Future Criminals. And it’s Biased Against Blacks. https://www.propublica.org/article/ machine-bias-risk-assessments-in-criminal-sentenc 2016.   
...
[20] J. Larson, S. Mattu, L. Kirchner, and J. Angwin. How g, We Analyzed the COMPAS Recidivism Algorithm. https://www.propublica.org/article/ how-we-analyzed-the-compas-recidivism-algorithm, 2016.   
...
[22] C. Mu˜noz, M. Smith, and D. Patil. Big Data: A Report on Algorithmic Systems, Opportunity, and Civil Rights. Executive Office of the President. The White House., 2016.   
