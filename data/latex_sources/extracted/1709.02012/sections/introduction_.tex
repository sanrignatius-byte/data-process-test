...
that these errors are made in ways that systematically discriminate against
...
news organization ProPublica, who studied a risk tool
...
defendants who didn't go in to commit future crimes were on average
...
crime in the future is a form of false-positive error, and in this case it
...
frameworks which attempt to quantify fairness in classification
\cite{barocas2016big,berk2017fairness,chouldechova2017fair,hardt2016equality,kleinberg2016inherent,woodworth2017learning,zafar2017fairness}.
A recent and particularly noteworthy framework is Equalized Odds \cite{hardt2016equality}
(also referred to as Disparate Mistreatment \cite{zafar2017fairness}),
...
However, equalized error rates do not necessarily remove all potential
for discrimination.
When risk tools are used in practice, a key goal is that they be
{\em calibrated}: if we look at the set of people who receive a probability
of $p$, we would like a $p$ fraction of the members of this set to
in fact be positive instances of the classification
problem \cite{dawid1982well}.
...
Calibration is a crucial condition for a risk tool in many settings.
If a risk tool for
evaluating defendants were not calibrated with respect to groups
defined by race, for example, then a probability estimate of $x$
could mean different things for African-American and white defendants,
...
interpreting its predictions.\footnote{There are many similar examples.
...
We do know from recent work that,
except in the most constrained
cases, it is impossible to achieve calibration while also
satisfying the Equalized Odds framework
...
that might be feasible in practice, or how calibration may be
combined with other definitions of fairness.

\noindent\textbf{Our Contributions.}
In this paper, our goal is to further investigate the relationship
...
groups with a surprisingly simple geometric intuition.
From this intuition, we can derive a number of straightforward
yet profound implications.
%
Firstly, while there are notions of error-rate non-discrimination that are compatible
with calibration, we show that such instances require a significant
relaxation of the equalized odds conditions.
Additionally, any such notions become infeasible when the prediction task
is inherently noisy, or when population group membership is a strong predictive
feature.
Finally, we demonstrate that achieving calibration and even the most simple
