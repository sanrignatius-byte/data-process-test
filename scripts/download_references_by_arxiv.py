...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-empty",
        action="store_true",
        help="Do not fail when 0 references are returned",
    )
    ...
    if len(records) == 0 and not args.allow_empty:
        print("Error: 0 references returned. This usually means API quota/network issue, or this paper has no indexed references.")
