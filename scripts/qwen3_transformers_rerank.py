#!/usr/bin/env python3
"""Transformers-only Qwen3 reranker for DPT.

This is the fallback path when vLLM import/load is blocked by the cluster
filesystem. It uses the same Qwen3-Reranker yes/no scoring recipe, but runs
plain AutoModelForCausalLM forward passes.
"""
...
def format_messages(instruction: str, query: str, doc: str) -> list[dict[str, str]]:
    return [
        {
            "role": "system",
            "content": (
...
def score_batch(model, tokenizer, prompts: list[str], true_token: int,
                false_token: int, max_length: int) -> list[float]:
    import torch
    ...
    with torch.inference_mode():
        ...
        pair_logits = torch.stack([logits[:, false_token], logits[:, true_token]], dim=1)
        probs = torch.softmax(pair_logits.float(), dim=1)[:, 1]
    return [float(x) for x in probs.detach().cpu()]
...
def main() -> None:
    ...
    print(f"[info] importing transformers/torch for {args.model}", flush=True)
    import torch
    ...
    print("[info] loading AutoModelForCausalLM", flush=True)
    ...
    for row_idx, row in enumerate(dense_rows, start=1):
        ...
        for start in range(0, len(pids), args.batch_size):
            ...
            prompts = [
                apply_chat(tokenizer, format_messages(args.instruction, qtext, corpus.get(pid, ""))) + suffix
                for pid in chunk
            ]
            scores = score_batch(model, tokenizer, prompts, true_token, false_token, args.max_length)
    ...
    metrics = {
        ...
        "qwen3_transformers_reranker": compute_metrics(reranked, qrels),
    }
