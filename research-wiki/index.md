...
- [exp:20260513_qc_prompt_v2_tightening](experiments/20260513_qc_prompt_v2_tightening.md) — **Completed 5/13**: Audit of `graph_max20k_four_cells_snapshot_20260513_utc` (6111 rows, 3251 pass / 53.2%) surfaced 8 high-prevalence pollution patterns; dominant is `the bridge X` narration in 83.3% of pass-set answers (Bridge label appears 22× in prompt, 1 rule says not to use it). Merged copilot prompt rewrite into claude branch + applied 4 follow-up fixes + ported `check_premise_contains_answer`. Result: **9 hard-fail code QC gates** wired into `qc_multihop_query`/`qc_real_user_query`; **Bridge vocabulary reduced 22 → 9** (all intentional: format placeholders + schema sentinels + forbidden-quote literals); 19-rule prompt v2 with anti-pattern block + opener variety + 6-way `query_type` enum. Tests 60/60 in `test_qc_checks.py`, 137/137 repo-wide. Retroactive audit: **96.9% of current pass set would fail under new gates**; predicted next-gen pass rate **53% → 80–85%**. Commits 1d65e80 + 1343f16 on `claude/review-triplet-learning-design-4dEas`. New claim [C12](/projects/_hdd/myyyx1/data-process-test/research-wiki/claims/C12_prompt_vocabulary_drives_narration_leak.md).
...
- [claim:C12](/projects/_hdd/myyyx1/data-process-test/research-wiki/claims/C12_prompt_vocabulary_drives_narration_leak.md): **Supported (2026-05-13)** — Prompt vocabulary density drives container-narration leakage in generated answers. When a scaffolding label word (e.g. "Bridge") appears N times in user-visible prompt prose, the LLM echoes it as a noun reference in answers at a frequency proportional to N. Evidence: original snapshot prompt had 22 visible `bridge*` mentions → 83.3% of pass-set answers contain `the bridge X` narration; reducing to 9 mentions + renaming the canonical slot to "Connecting paragraph" is the prompt-side fix. End-to-end validation pending the next 50-candidate smoke run.
